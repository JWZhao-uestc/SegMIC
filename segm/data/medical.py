...
class MedicalDataset(Dataset):
        ...
        if db_name == 'all':  
            if split == 'train':
                ...
                for task in total_tasks:
                    ...
                    if os.path.isdir(join(self.gt_path, task)) and '.' not in task: 
                        total_train_imgs += glob(join(self.gt_path, task, 'Mask/NEW_Mask_png/', split, '*/*', '*.png'))
                self.imglist = total_train_imgs   
            if split == 'test':
                total_train_imgs = []
                total_tasks = ['not_train/ACDC']#['not_train/spineWeb']#['not_train/PanDental']
                for task in total_tasks:
                    if os.path.isdir(join(self.gt_path, task)) and 'Brain' not in task and '.' not in task: 
                        total_train_imgs += glob(join(self.gt_path, task, 'Mask/NEW_Mask_png/', split, '*/*', '*.png'))
                self.imglist = total_train_imgs
    ...
    def __getitem__(self, idx):
        ...
        while object_aera <= mask_aera_min or object_aera == (self.image_size // 16)**2: # all 0 or all 1
            ...
            # ep_label_32 = cv2.resize(ep_label, (self.image_size // 16, self.image_size // 16), interpolation = cv2.INTER_NEAREST) #####resize to 32x32, must still has value 1
            # ep_label_32 = (ep_label_32 >= (255 // 2)) * 255
            # object_aera = np.sum(ep_label_32 // 255)
        ...
        if self.transform is not None:
            ...
            #ep_label_32 = cv2.resize(ep_label.cpu().detach().numpy(), (self.image_size // 16, self.image_size // 16), interpolation = cv2.INTER_NEAREST)
            # cv2.imwrite('ep_mask.png', ep_label.cpu().detach().numpy()*255)
            # cv2.imwrite('ep_mask_32.png', ep_label_32*255)
            #ep_label_32 = torch.from_numpy(ep_label_32)
            #task_transform
        ...
        return {'im':image.float(), 'gt':label.float(), 
                'ep_im':ep_image.float(), 'ep_gt':ep_label.float(), 'ep_gt_32': ep_label.float(),
