...
class MedicalDataset(Dataset):
            ...
            if split == 'train':
                ...
                for task in total_tasks:
                    if os.path.isdir(join(self.gt_path, task)) and 'Brain' not in task and '.' not in task: 
                        mask_and_edge = glob(join(self.gt_path, task, 'Mask/NEW_Mask_png/', split, '*/*', '*.png')) + glob(join(self.gt_path, task, 'Mask/NEW_Sobel_png/', split, '*/*', '*.png')) 
                        total_train_imgs += mask_and_edge
    ...
    def __getitem__(self, idx):
        ...
        image_path = self.imglist[idx].replace('Mask', 'Image').replace('Sobel', 'Image')
        ...
        while object_aera <= mask_aera_min or object_aera == (self.image_size // 16)**2: # all 0 or all 1
            ...
            ep_label_count_path = ep_label_path.replace('Sobel', 'Mask') #####
            label_x = ep_label_path.split('/')[-2]
            ep_image_path = ep_label_path.replace('Mask', 'Image').replace('Sobel', 'Image').replace(label_x+'/', '')
            ...
            if 'Sobel' in ep_label_path:
                # find corresponding mask to calculate aera
                ep_image_count = cv2.imread(ep_label_count_path, cv2.IMREAD_GRAYSCALE) #####
                ep_image_count = (ep_image_count >= (255 // 2)) * 255
                object_aera = np.sum(ep_image_count // 255)
            else:
