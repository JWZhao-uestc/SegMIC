...
def inference_binary(
    ...
    b,c,h,w = im.shape
    final_val = np.zeros((b, 2, h, w))
    counts = np.zeros((b, 1, h, w))
    global_val = np.zeros((b, h, w))
    stride = window_stride
    hs = 0
    gim = F.interpolate(im, (window_size, window_size), mode='bilinear', align_corners=True)
    gc = 0
    while hs <= h - window_size:
        he = hs + window_size
        ws = 0
        while ws <= w - window_size:
            we = ws + window_size
            lim = im[:, :, hs:he, ws:we]
            gs_val, ls_val = model(gim, lim)
            final_val[:, :, hs:he, ws:we] += torch.softmax(ls_val,dim=1).detach().cpu().numpy()
            counts[:, :, hs:he, ws:we] += 1
            ws = ws + stride
            gs_val = torch.softmax(gs_val,dim=1)
            gs_val = F.interpolate(gs_val,(h,w),mode='bilinear', align_corners=True)
            global_val += gs_val.detach().cpu().numpy()[:,1]
            gc+=1
        hs = hs + stride
    final_val = final_val / counts
    global_val = global_val / gc
    seg_map = final_val[:,1]
    return seg_map, global_val
...
def inference_bce(
    ...
    while hs <= h - window_size:
        ...
        while ws <= w - window_size:
            we = ws + window_size
            lim = im[:, :, hs:he, ws:we]
            gs_val, ls_val, rs_val = model(gim, lim)
            final_val[:, :, hs:he, ws:we] += torch.sigmoid(rs_val).detach().cpu().numpy()
            counts[:, :, hs:he, ws:we] += 1
            ws = ws + stride
            gs_val = torch.softmax(gs_val,dim=1)
            gs_val = F.interpolate(gs_val,(h,w),mode='bilinear', align_corners=True)
...
def inference_medsam(
    ...
    with torch.no_grad():
        import time
        start = time.time()
        query_seg_pred, ep_seg_pred, query_x, ep_x, query_mask_embed_, ep_mask_embed_, _ = model.forward(im,ep_im,seg_gt,ep_seg_gt,half_mask) ###
        end = time.time()
    # from thop import profile
    # flops, params = profile(model, inputs=(im,ep_im,seg_gt,ep_seg_gt,half_mask), verbose=False)

    # print(f"GFLOPs: {flops / 1e9:.3f}")
    # print(f"Params: {params / 1e6:.3f} M")
    print(f"Time: {end-start}")
    #exit()
    return query_seg_pred, ep_seg_pred, query_x, ep_x, query_mask_embed_, ep_mask_embed_
