...
def inference_bce(
    ...
    while hs <= h - window_size:
        ...
        while ws <= w - window_size:
            ...
            lim = im[:, :, hs:he, ws:we]
            gs_val, ls_val = model(gim, lim)
            final_val[:, :, hs:he, ws:we] += torch.sigmoid(ls_val).detach().cpu().numpy()
            counts[:, :, hs:he, ws:we] += 1
            ws = ws + stride
            gs_val = torch.torch.sigmoid(gs_val)
            gs_val = F.interpolate(gs_val,(h,w),mode='bilinear', align_corners=True)
            global_val += gs_val.detach().cpu().numpy()
            gc+=1
        hs = hs + stride
    final_val = final_val / counts
    global_val = global_val / gc
