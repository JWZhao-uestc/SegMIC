...
class VisionTransformer(nn.Module):
    ...
    def forward(self, gim, lim, return_features=False):
        B, _, H, W = gim.shape
        PS = self.patch_size

        gx = self.patch_embed(gim)
        lx = self.patch_embed(lim)
        ...
        if gx.shape[1] != pos_embed.shape[1]:
        ...
        if lx.shape[1] != pos_embed.shape[1]:
            pos_embed = resize_pos_embed(
                pos_embed,
                self.patch_embed.grid_size,
                (H // PS, W // PS),
        ...
        x = x + torch.cat([pos_embed,local_embed],dim=1)
        x = self.dropout(x)
